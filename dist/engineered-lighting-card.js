...
class EngineeredLightingCard extends HTMLElement {
      ...
      <div class="root">
          ...
          ${cams.map((cam, i) => `
            <div class="cam-cell" id="cell-${cam.name}">
              ...
              <div class="pipeline-bar" id="pipe-${cam.name}">
                ...
                <div class="pipe-arrow" id="pipe-arrow-vjepa-${cam.name}">&rarr;</div>
                <div class="pipe-stage">
        ...
        <div class="metrics-section">
          <div class="metrics-row">
            ...
            <!-- Frigate / LattePanda -->
            <div class="m-card">
              <div class="m-hdr"><span class="m-icon frigate-icon">&#9672;</span><span class="m-title">Frigate NVR</span><span class="m-badge" id="m-frigate-badge">&mdash;</span></div>
              <div class="m-grid">
                <div class="m-item"><span class="m-label">CPU</span><div class="m-bar"><div class="m-bar-fill cpu-fill" id="m-f-cpu-bar"></div></div><span class="m-val" id="m-f-cpu">&mdash;</span></div>
                <div class="m-item"><span class="m-label">Memory</span><div class="m-bar"><div class="m-bar-fill mem-fill" id="m-f-mem-bar"></div></div><span class="m-val" id="m-f-mem">&mdash;</span></div>
                <div class="m-item"><span class="m-label">Uptime</span><span class="m-val" id="m-f-uptime">&mdash;</span></div>
              </div>
            </div>

            <!-- Coral TPU -->
            <div class="m-card">
              <div class="m-hdr"><span class="m-icon coral-icon">&#9650;</span><span class="m-title">Coral TPU</span><span class="m-badge" id="m-coral-badge">&mdash;</span></div>
              <div class="m-grid">
                <div class="m-item"><span class="m-label">Inference</span><span class="m-val" id="m-c-speed">&mdash;</span></div>
                <div class="m-item"><span class="m-label">Temperature</span><span class="m-val temp" id="m-c-temp">&mdash;</span></div>
                <div class="m-item"><span class="m-label">Detection</span><span class="m-val" id="m-c-detect">&mdash;</span></div>
              </div>
            </div>
            ...
            <div class="m-card">
              ...
              <div class="m-grid">
                <div class="m-item"><span class="m-label">CPU</span><div class="m-bar"><div class="m-bar-fill cpu-fill" id="m-j-cpu-bar"></div></div><span class="m-val" id="m-j-cpu">&mdash;</span></div>
                <div class="m-item"><span class="m-label">GPU</span><div class="m-bar"><div class="m-bar-fill gpu-fill" id="m-j-gpu-bar"></div></div><span class="m-val" id="m-j-gpu">&mdash;</span></div>
                <div class="m-item"><span class="m-label">RAM</span><div class="m-bar"><div class="m-bar-fill mem-fill" id="m-j-ram-bar"></div></div><span class="m-val" id="m-j-ram">&mdash;</span></div>
                <div class="m-item"><span class="m-label">CPU Temp</span><span class="m-val temp" id="m-j-ctemp">&mdash;</span></div>
                <div class="m-item"><span class="m-label">GPU Temp</span><span class="m-val temp" id="m-j-gtemp">&mdash;</span></div>
              </div>
            </div>
            ...
            <div class="m-card">
              ...
              <div class="m-grid">
                <div class="m-item"><span class="m-label">FPS</span><span class="m-val" id="m-v-fps">&mdash;</span></div>
                <div class="m-item"><span class="m-label">Latency</span><span class="m-val" id="m-v-latency">&mdash;</span></div>
                <div class="m-item"><span class="m-label">Frames</span><span class="m-val" id="m-v-frames">&mdash;</span></div>
                <div class="m-item"><span class="m-label">Active Cams</span><span class="m-val" id="m-v-cams">&mdash;</span></div>
              </div>
            </div>
  ...
  _updateMetrics() {
    ...
    if (st.service) {
      ...
      const ue = $('m-f-uptime');
      if (ue) ue.textContent = hrs > 0 ? `${hrs}h ${min}m` : `${min}m`;
      const be = $('m-frigate-badge');
    ...
    if (st.cpu_usages) {
      ...
      if (fs) {
        const ce = $('m-f-cpu');
        if (ce) ce.textContent = (fs.cpu || 0) + '%';
        bar('m-f-cpu-bar', fs.cpu);
        const me = $('m-f-mem');
        if (me) me.textContent = (fs.mem || 0) + '%';
        bar('m-f-mem-bar', fs.mem);
      }
    }
    ...
    if (st.detectors) {
      ...
      if (det) {
        const se = $('m-c-speed');
        if (se) se.textContent = (det.inference_speed || 0).toFixed(1) + ' ms';
        const de = $('m-c-detect');
        if (de) de.textContent = det.detection_start ? det.detection_start.toFixed(1) + ' ms' : 'Idle';
    ...
    if (st.temperatures) {
      ...
      if (temp) {
        const te = $('m-c-temp');
        if (te) {
          te.textContent = temp.toFixed(1) + '\u00B0C';
          te.className = 'm-val temp ' + this._tmpCls(temp);
        }
      }
    }
    ...
    // RAM: use the working entity with ram_used_mb/ram_total_mb attributes
    const jRam = h.states['sensor.jetson_orin_nano_jetson_ram_usage'];
    if (jRam && jRam.state !== 'unavailable' && jRam.state !== 'unknown') {
      ...
      const e = $('m-j-ram');
      if (e) e.textContent = `${used}/${tot} GB`;
      bar('m-j-ram-bar', pct);
    }
    ...
    if (jCt && jCt.state !== 'unavailable') {
      const e = $('m-j-ctemp');
      if (e) {
        e.textContent = jCt.state + '\u00B0C';
        e.className = 'm-val temp ' + this._tmpCls(parseFloat(jCt.state));
      }
    }
    ...
    if (jGt && jGt.state !== 'unavailable') {
      const e = $('m-j-gtemp');
      if (e) {
        e.textContent = jGt.state + '\u00B0C';
        e.className = 'm-val temp ' + this._tmpCls(parseFloat(jGt.state));
      }
    }

    const jSt = h.states['sensor.jetson_status'];
    if (jSt && jSt.state !== 'unavailable' && jSt.state !== 'unknown') {
      const e = $('m-jetson-badge');
      if (e) { e.textContent = 'Online'; e.className = 'm-badge badge-on'; }
    }

    // — V-JEPA 2 —
    const vMap = {
      'sensor.v_jepa_2_fps': 'm-v-fps',
      'sensor.v_jepa_2_inference_latency': 'm-v-latency',
      'sensor.v_jepa_2_frames_processed': 'm-v-frames',
      'sensor.v_jepa_2_active_cameras': 'm-v-cams'
    };
    ...
    if (vSt && vSt.state !== 'unavailable' && vSt.state !== 'unknown') {
      const e = $('m-vjepa-badge');
      if (e) { e.textContent = vSt.state; e.className = 'm-badge badge-on'; }
    }
  }
  ...
  _css() { return `
      ...
      * { box-sizing: border-box; margin: 0; padding: 0; }
      ...
      .cam-cell:nth-child(4), .cam-cell:nth-child(5) { }
...
window.customCards.push({
  ...
  description: 'V-JEPA 2 World Model Dashboard'
});
